import { useState, useEffect } from 'react'
import { fetchFathers } from '@/app/api/apis/fathers'

export const Fathers = () => {
  const [fathers, setFathers] = useState([])
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState(null)

  useEffect(() => {
    const fetchData = async () => {
      setLoading(true)
      setError(null)
      try {
        const response = await fetchFathers()
        if (Array.isArray(response)) {
          setFathers(response)
        } else {
          setError('Datos inválidos recibidos del servidor')
        }
      } catch (err) {
        console.error('Error fetching fathers: ', err)
        setError('No se pudo cargar la lista de padres')
      } finally {
        setLoading(false)
      }
    }

    fetchData()
  }, [])

  if (loading) {
    return <div className="text-center text-white">Cargando padres...</div>
  }

  if (error) {
    return <div className="text-center text-red-500">{error}</div>
  }

  return (
    <div className="container mx-auto px-4 pb-16">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold text-white">Padres</h1>
      </div>
      {fathers.length === 0 ? (
        <p className="text-center text-slate-400">No hay padres registrados.</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {fathers.map((father) => (
            <div
              key={father.id}
              className="bg-slate-700 rounded-lg shadow-md overflow-hidden transition-transform duration-300 ease-in-out hover:scale-105"
            >
              <img
                src={father.imageUrl || '/default-image.png'}
                alt={`Foto de ${father.first_name || 'Padre'}`}
                className="w-full h-48 object-cover"
              />
              <div className="p-4">
                <h2 className="text-xl font-semibold text-slate-200">
                  {father.first_name} {father.last_name}
                </h2>
                <p className="text-slate-400">{father.email || 'Sin correo'}</p>
                <p className="text-slate-400">{father.phone || 'Sin teléfono'}</p>
                <p className="text-slate-300 mt-2">Edad: {father.age || 'N/A'}</p>
                {father.activo && (
                  <div className="bg-green-500 mt-2 rounded-lg w-2 h-2"></div>
                )}
              </div>
            </div>
          ))}
        </div>
      )}
    </div>
  )
}
